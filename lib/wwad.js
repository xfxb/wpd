...
var Wwad = function () {
  function Wwad(options) {
    ...
    var public_path = cwd || process.cwd();
    var configjs = path.resolve(public_path, './wwad.config.js');
    var configJsObj = null;

    if (isFsExistsSync(configjs)) {
      var config = require(configjs); // eslint-disable-line
